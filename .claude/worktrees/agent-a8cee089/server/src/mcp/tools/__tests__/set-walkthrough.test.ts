...
describe('set_walkthrough', () => {
  ...
  it('stores walkthrough via applyEvent on valid input', async () => {
    ...
    const [calledPrKey, event] = applyEventMock.mock.calls[0] as [string, SessionEvent];
    expect(calledPrKey).toBe('pr-1');
    expect(event.type).toBe('walkthrough.set');
    if (event.type === 'walkthrough.set') {
      expect(event.walkthrough.steps).toHaveLength(1);
      expect(event.walkthrough.steps[0]).toMatchObject({
        stepNum: 1,
        hunkId: 'f1:h0',
        commentary: 'Test commentary',
        status: 'pending',
      });
      expect(event.walkthrough.cursor).toBe(0);
      expect(event.walkthrough.showAll).toBe(false);
    }
  });

  it("sets all step statuses to 'pending'", async () => {
    // Need 3 hunks for this test — add more hunks to the session
    ...
    sessions.set('pr-1', {
      ...session,
      diff: { files: [extraFile], totalHunks: 3 },
    });

    const res = await handler({
      prKey: 'pr-1',
      steps: [
        { hunkId: 'f1:h0', commentary: 'Step 1' },
        { hunkId: 'f1:h1', commentary: 'Step 2' },
        { hunkId: 'f1:h2', commentary: 'Step 3' },
      ],
    });
    expect(res.isError).toBeUndefined();

    const [, event] = applyEventMock.mock.calls[0] as [string, SessionEvent];
    if (event.type === 'walkthrough.set') {
      expect(event.walkthrough.steps).toHaveLength(3);
      for (const step of event.walkthrough.steps) {
