...
**Domain:** Claude Code plugin with local web GUI for LLM-assisted GitHub PR reviews, driven by MCP tools
**Researched:** 2026-04-16
**Confidence:** HIGH on plugin layout, MCP SDK, web stack, runtime, SQLite, Octokit, transport. MEDIUM on diff-rendering choice (defensible alternative exists). MEDIUM on syntax highlighter (one genuine tradeoff).
...
```bash
# MCP server side
pnpm add @modelcontextprotocol/sdk zod hono @hono/node-server ws better-sqlite3 octokit execa open get-port nanoid parse-diff
pnpm add -D typescript tsx vitest @types/node @types/ws @types/better-sqlite3

# Web app side (separate workspace)
pnpm add react react-dom @git-diff-view/react shiki
...
├── commands/
│   ├── review.md                       # /git-review-plugin:review <pr-url | ref1..ref2>
│   └── resume.md                       # /git-review-plugin:resume
├── .mcp.json                           # declares the bundled MCP server
├── server/                             # MCP + Hono + WS process
│   ├── src/index.ts                    # McpServer + StdioServerTransport; also boots Hono+WS on a free port
│   ├── src/tools/                      # show_hunk, post_comment, run_self_review, submit_review, …
│   └── package.json
├── web/                                # Vite + React SPA
│   ├── src/
│   ├── dist/                           # built static assets served by Hono
│   └── package.json
├── shared/
│   └── types.ts                        # WS message types + MCP tool payload types
└── bin/                                # nothing needed here; the MCP server is started via .mcp.json
```
...
```json
{
  "mcpServers": {
    "git-review": {
      ...
      "env": {
        "REVIEW_DATA_DIR": "${CLAUDE_PLUGIN_DATA}"
      }
    }
  }
}
...
```json
{
  ...
  "version": "0.1.0",
  "description": "Pair-review pull requests with Claude in a local web GUI"
}
```

Slash command `commands/review.md` frontmatter:
```markdown
---
description: Open a PR or local ref range in the review workspace
---

# Review

Start a review session for "$ARGUMENTS". Use the git-review MCP tools to drive the workflow.
```
...
The Claude Code plugin spec is public, current, and narrow. Manifest lives at `.claude-plugin/plugin.json`; bundled MCP servers are declared in `.mcp.json` at plugin root using `${CLAUDE_PLUGIN_ROOT}` path substitution; slash commands are markdown files under `commands/` (each becomes `/<plugin-name>:<command-name>`); hooks live in `hooks/hooks.json`. Use `${CLAUDE_PLUGIN_DATA}` for the SQLite file so state survives plugin updates.

### 2. MCP server implementation — HIGH confidence
Use `@modelcontextprotocol/sdk@1.29.0` with `McpServer` + `StdioServerTransport`. **Stdio, not Streamable HTTP.** Rationale: the plugin is spawned by Claude Code as a child process (per `.mcp.json`), stdio is zero-config, there's no cross-host scenario, and HTTP would require a second auth surface. Register tools with `server.registerTool(name, { description, inputSchema: { ... zod schemas } }, handler)` and return `{ content: [{ type: "text", text: ... }] }`. **Critical:** never use `console.log` — only `console.error` — because stdout is the JSON-RPC channel.

### 3. Local web app — HIGH confidence
**Vite + React SPA served by Hono, same process as the MCP server.** Not Next.js (SSR/file-based routing buy nothing for a local no-SEO tool and complicate shipping inside a plugin), not Remix (same reason, plus Remix has folded into React Router so the framing is out of date), not Express (larger, no WebSocket helpers, worse DX in 2026). Hono is tiny, Web-Standards based (so Request/Response work unchanged between Node/Bun), has a first-class WebSocket upgrade API, and can serve the built SPA's `dist/` as static content in the same process. This collapses to one process: stdio JSON-RPC to Claude Code on one side, HTTP+WS to the browser on the other, shared in-memory state.
...
`@git-diff-view/react` is the right call: it consumes unified diff output directly (which is exactly what both `gh pr diff` and local `git diff` produce — single pipeline), renders a GitHub-style split or unified view, exposes a token system for inline word-level diffs, supports web-worker rendering for large PRs, and provides the hooks needed to anchor inline comments to (file, line). Actively maintained (release 2026-03-19). React 19 peer.
...
**Use `gh` CLI for:** auth (inherit user's existing login — no second token flow), fetching PR metadata (`gh pr view --json`), fetching the diff (`gh pr diff`), fetching existing comments (`gh api repos/{owner}/{repo}/pulls/{num}/comments`). This is PROJECT.md's stated expectation and avoids re-implementing `gh`'s token-store behavior.

**Use `octokit@5.0.5` for:** the final review submission — `octokit.rest.pulls.createReview({ owner, repo, pull_number, event, body, comments })` — because it's a single typed call with batched inline comments, and building the multi-part review payload over `gh api` curl-style is ugly. The REST endpoint `POST /repos/{owner}/{repo}/pulls/{pull_number}/reviews` accepts `event: APPROVE | REQUEST_CHANGES | COMMENT`, `body`, and `comments: [{ path, position, body }]` in one request — sufficient. GraphQL `addPullRequestReview` is equivalent but more verbose; no v1 need.

Token plumbing: if `gh` is logged in, read the token via `gh auth token` and pass it to Octokit (`new Octokit({ auth: token })`). Fall back to `GITHUB_TOKEN` / `GH_TOKEN` env vars.

### 6. Local-branch diff mode — HIGH confidence (shell out + `parse-diff`)
**Shell out to `git diff <ref1>...<ref2>` via `execa`, then feed the output to `parse-diff`.** Same unified-diff pipeline as GitHub mode — the diff rendering path only ever sees parsed hunks; it doesn't care where they came from.
...
Use `ws@8.20.0` — minimal, fast, Hono has a clean `upgradeWebSocket` helper. The control plane is inherently bidirectional: the LLM's MCP tool handler calls `show_hunk(file, line)`, which needs to push to the browser AND wait for an ACK/result before the MCP tool call resolves. SSE is one-way server→client and would force you to layer a separate POST channel for ACKs. Socket.IO is overkill for a single local connection (reconnection, rooms, fallbacks, and 100kB+ of client code you don't need).

### 8. State persistence — HIGH confidence (`better-sqlite3`)
**One SQLite file at `${CLAUDE_PLUGIN_DATA}/state.db`**, schema roughly: `reviews(pr_key, status, current_hunk, created_at)`, `comments(review_id, path, line, body, draft)`, `walkthrough_steps(review_id, ordinal, hunk_key)`, `checklist_results(review_id, item, criticality, verdict, note)`.

`better-sqlite3@12.9.0` is synchronous (simpler for a single-user tool), native-compiled (fast), and the de facto standard. **Do not use Node's built-in `node:sqlite`:** still marked experimental in Node 22/24 and requires `--experimental-sqlite`.
...
**Node.js 22 LTS.** Rationale: Claude Code itself runs on Node, the plugin will be spawned by whatever Node the user has, `better-sqlite3` supports `20.x || 22.x || 23.x || 24.x || 25.x`, and the MCP SDK requires `>=18`. Using Bun would force users to install Bun to run your plugin — a non-trivial ask for a personal tool that needs to "just work" when installed. Keep Bun as a dev-only option if you like (`bun test` is faster than `vitest`), but ship to Node.

### 10. Syntax highlighting — MEDIUM confidence (Shiki, with a real tradeoff)
**Shiki 4.x for correctness; pre-render server-side and cache.** Shiki uses the same TextMate grammars VS Code ships, so highlighting matches the editor the author uses daily. The practical concern is bundle/weight: loading every language in the browser balloons the SPA. Mitigation: run Shiki on the server (inside the plugin process), per hunk, on-demand, and cache the highlighted HTML keyed by `(file-path, git-blob-sha, language)`. Send the highlighted tokens over WS as part of the hunk payload.

Alternative if you want pure client-side and smaller bundle: `highlight.js@11` via `lowlight` — which is already a transitive dep of `@git-diff-view/react`, so you could simply use what's bundled. Less accurate, smaller, zero extra work. Reasonable v1 default; upgrade to Shiki if highlighting quality becomes a papercut.

Avoid `prismjs` — maintained but visibly falling behind Shiki/highlight.js in 2026.
...
**If you later want to open this tool to a team or publish it:**
...
**If review responsiveness on very large PRs (>1000 changed lines) becomes an issue:**
...
**If you want to drop the `gh` CLI dependency (e.g., for CI sandboxes):**
...
1. **Syntax highlighter final call** — Shiki (server-side) vs piggyback on `lowlight`/`highlight.js` already transited through `@git-diff-view/react`. Defer to the first render phase; a 30-minute spike will decide based on perceived quality on a real PR.
2. **Plugin publishing path** — PROJECT.md says "Polished open-source release…deferred." So `marketplace.json` and submission to `claude.ai/settings/plugins/submit` are intentionally out of scope for v1; the plugin loads via `claude --plugin-dir ./` during development.
3. **Subagent usage** — PROJECT.md mentions subagents are available; this stack doesn't require one. A later phase may add a read-only `checklist-runner` subagent if the main-thread context cost of running all checklist items inline becomes a problem.
...
---
*Stack research for: Claude Code plugin with local web GUI for LLM-assisted GitHub PR reviews*
*Researched: 2026-04-16*
