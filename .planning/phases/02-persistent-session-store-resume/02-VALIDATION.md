...
| 2-02-01 | 02 | 1 | SESS-03 | T-2-02-00 | Optional `lockOptions` parameter on `writeState` widens retries in test-side only; production-tight default preserved; existing 2-arg callers unaffected | unit | `cd server && pnpm test -- store.test.ts --run` | ❌ W0 (server/src/persist/store.ts signature widening + new tests in existing store.test.ts) | ⬜ pending |
| 2-02-02 | 02 | 1 | SESS-03 | T-2-02-01, T-2-02-02 | SIGKILL during writeState leaves state.json either pristine or advanced — never truncated (5 iterations with varying timing) | integration | `cd server && pnpm test -- store.crash.test.ts --run` | ❌ W0 (server/src/persist/__tests__/store.crash.test.ts) | ⬜ pending |
| 2-02-03 | 02 | 1 | SESS-03 | — | Concurrent same-prKey writes serialize cleanly via proper-lockfile (widened WIDE_LOCK budget via new parameter); different-prKey writes parallelize under default budget | integration | `cd server && pnpm test -- store.concurrency.test.ts --run` | ❌ W0 (server/src/persist/__tests__/store.concurrency.test.ts) | ⬜ pending |
| 2-02-04 | 02 | 1 | SESS-03 | — | Stale lockdir (30s old) broken within 3s under widened WIDE_LOCK budget (production-tight 150ms default is categorically insufficient for proper-lockfile stale-detection); new writeState succeeds | integration | `cd server && pnpm test -- store.stale-lock.test.ts --run` | ❌ W0 (server/src/persist/__tests__/store.stale-lock.test.ts) | ⬜ pending |
...
- [ ] `server/src/persist/store.ts` — MODIFIED (Plan 02 Task 1) — optional `lockOptions` parameter added; defaults preserved
- [ ] `server/src/persist/__tests__/store.test.ts` — EXTENDED (Plan 02 Task 1) — 3 new tests for the signature widening
- [ ] `server/src/persist/__tests__/store.crash.test.ts` — NEW (Plan 02 Task 2)
- [ ] `server/src/persist/__tests__/crash-fixture.ts` — NEW (Plan 02 Task 2)
- [ ] `server/src/persist/__tests__/store.concurrency.test.ts` — NEW (Plan 02 Task 3)
- [ ] `server/src/persist/__tests__/store.stale-lock.test.ts` — NEW (Plan 02 Task 4)
